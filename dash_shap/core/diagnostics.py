...
class ImportanceStabilityPlot:
    ...
    def plot(
        ...
        xlim = ax.get_xlim()
        ylim = ax.get_ylim()
        ax.fill_betweenx([ylim[0], fsi_threshold], xlim[0], importance_threshold,
                         alpha=0.04, color="#95a5a6", zorder=0)   # Q-III gray
        ax.fill_betweenx([fsi_threshold, ylim[1]], xlim[0], importance_threshold,
                         alpha=0.04, color="#f39c12", zorder=0)   # Q-IV orange
        ax.fill_betweenx([ylim[0], fsi_threshold], importance_threshold, xlim[1],
                         alpha=0.04, color="#2ecc71", zorder=0)   # Q-I green
        ax.fill_betweenx([fsi_threshold, ylim[1]], importance_threshold, xlim[1],
                         alpha=0.04, color="#e74c3c", zorder=0)   # Q-II red
        ax.set_xlim(xlim)
        ax.set_ylim(ylim)
        xp = 0.015 * (xlim[1] - xlim[0])
        yp = 0.025 * (ylim[1] - ylim[0])
        ax.text(importance_threshold + xp, ylim[0] + yp,
                "Q-I: Robust", fontsize=7, color="#1a8a47", va="bottom", alpha=0.8)
        ax.text(importance_threshold + xp, fsi_threshold + yp,
                "Q-II: Collinear", fontsize=7, color="#a93226", va="bottom", alpha=0.8)
        ax.text(xlim[0] + xp, ylim[0] + yp,
                "Q-III: Unimportant", fontsize=7, color="#5d6d7e", va="bottom", alpha=0.8)
        ax.text(xlim[0] + xp, fsi_threshold + yp,
                "Q-IV: Fragile", fontsize=7, color="#b7770d", va="bottom", alpha=0.8)
