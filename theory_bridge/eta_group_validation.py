...
if __name__ == "__main__":
    ...
    for pair_idx, pair_name in enumerate(["pair1 (f0,f1)", "pair2 (f2,f3)"]):
        ...
        print(
            f"  {pair_name}: inv_flip={r['mean_invariant_flip']:.4f}, var_flip={r['mean_variant_flip']:.4f}, ratio={r['ratio']:.1f}x, p={r['wilcoxon_p']:.2e}"
        )
    ...
    print(
        f"  Lat/Long: inv_flip={r['mean_invariant_flip']:.4f}, var_flip={r['mean_variant_flip']:.4f}, ratio={r['ratio']:.1f}x, p={r['wilcoxon_p']:.2e}"
    )

    r = test_group(shap_stack, [rooms_idx, bedrms_idx], "Rooms/Bedrms")
    all_results.append(("california_RoomsBedrms", r))
    print(
        f"  Rooms/Bedrms: inv_flip={r['mean_invariant_flip']:.4f}, var_flip={r['mean_variant_flip']:.4f}, ratio={r['ratio']:.1f}x, p={r['wilcoxon_p']:.2e}"
    )
    ...
    for bm_idx, bm in enumerate(base_measurements[:5]):  # test first 5
        ...
        print(
            f"  {bm}: inv_flip={r['mean_invariant_flip']:.4f}, var_flip={r['mean_variant_flip']:.4f}, ratio={r['ratio']:.1f}x, p={r['wilcoxon_p']:.2e}"
        )
    ...
    for i, j, name in diabetes_groups:
        ...
        print(
            f"  {name}: inv_flip={r['mean_invariant_flip']:.4f}, var_flip={r['mean_variant_flip']:.4f}, ratio={r['ratio']:.1f}x, p={r['wilcoxon_p']:.2e}"
        )
    ...
    print("=" * 70)
    print(
        f"{'Test':>35s}  {'g':>3s}  {'eta':>5s}  {'Inv flip':>9s}  {'Var flip':>9s}  {'Ratio':>6s}  {'p-value':>9s}  {'Pass?':>5s}"
    )
    print("-" * 95)
    for name, r in all_results:
        passed = "YES" if r["invariant_more_stable"] and r["wilcoxon_p"] < 0.05 else "NO"
        print(
            f"{r['group']:>35s}  {r['g']:>3d}  {r['eta_predicted']:>5.2f}  {r['mean_invariant_flip']:>9.4f}  {r['mean_variant_flip']:>9.4f}  {r['ratio']:>6.1f}x  {r['wilcoxon_p']:>9.2e}  {passed:>5s}"
        )
