...
Bold indicates best. Stability is a single scalar (mean pairwise Spearman);
Accuracy and Equity report mean $\pm$ SE across repetitions. At
$\rhovar = 0.9$ all methods are shown; at other levels, only Single Best
and \dash{} are compared in this table. See Table~\ref{tab:extended} for
the full baseline comparison at $\rhovar = 0.9$.}
\label{tab:sweep}
\small
...
$\rhovar = 0.9$ is presented in Table~\ref{tab:extended}. At the time
of writing, Stochastic Retrain has not been run across all $\rhovar$
levels; this is a limitation of the current experimental scope.
...
all metrics. We note that this baseline uses fixed hyperparameters
(max\_depth=6, learning\_rate=0.1) and is not tuned, so its poor
performance partly reflects the lack of tuning rather than purely the
sequential residual dependency mechanism. Nevertheless, the comparison
illustrates the risk of naively scaling up a single ensemble:
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Incomplete baseline sweep.} The correlation sweep
    (Table~\ref{tab:sweep}) does not include all baselines at all
    $\rhovar$ levels. Running Stochastic Retrain and Naive Averaging
    across the full sweep would strengthen the analysis.
  \item \textbf{Confidence intervals.} Due to computational constraints,
    the current benchmark uses $N_{\text{reps}} = 20$, which provides
    limited statistical power for detecting small effects. The Wilcoxon
    test is underpowered for the small effect sizes observed between
    \dash{} and Stochastic Retrain.
\end{itemize}
