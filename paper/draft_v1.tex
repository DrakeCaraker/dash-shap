...
\author{
  ...
  \texttt{puremath86@gmail.com} \quad \texttt{drhoads9@gmail.com}
}
...
\midrule
\multirow{4}{*}{0.0}
  & Single Best          & $.976 \pm .004$ & .987 & .152 & .599 \\
  & Large Single Model   & $.965 \pm .005$ & .981 & .155 & .745 \\
  & Stoch.\ Retrain      & $.974 \pm .004$ & .986 & .153 & .591 \\
  & \textbf{DASH (MaxMin)} & $\mathbf{.977} \pm .003$ & \textbf{.987} &
    \textbf{.150} & \textbf{.585} \\
\midrule
\multirow{4}{*}{0.5}
  & Single Best          & $.978 \pm .003$ & .989 & .165 & .609 \\
  & Large Single Model   & $.970 \pm .004$ & --- & .180 & --- \\
  & Stoch.\ Retrain      & $.979 \pm .003$ & .990 & .157 & .588 \\
  & \textbf{DASH (MaxMin)} & $\mathbf{.982} \pm .003$ & \textbf{.991} &
    \textbf{.151} & \textbf{.582} \\
\midrule
\multirow{4}{*}{0.7}
  & Single Best          & $.970 \pm .004$ & .984 & .197 & .609 \\
  & Large Single Model   & $.964 \pm .005$ & --- & .202 & --- \\
  & Stoch.\ Retrain      & $.978 \pm .003$ & .989 & .165 & .586 \\
  & \textbf{DASH (MaxMin)} & $\mathbf{.980} \pm .003$ & \textbf{.990} &
    \textbf{.154} & \textbf{.584} \\
\midrule
\multirow{4}{*}{0.9}
  & Single Best          & $.960 \pm .005$ & .980 & .217 & .604 \\
  & Large Single Model   & $.940 \pm .007$ & .969 & .253 & .718 \\
  & Stoch.\ Retrain      & $.980 \pm .003$ & .989 & .172 & .584 \\
  & \textbf{DASH (MaxMin)} & $\mathbf{.981} \pm .003$ & \textbf{.990} &
    \textbf{.163} & \textbf{.582} \\
\midrule
\multirow{4}{*}{0.95}
  & Single Best          & $.953 \pm .006$ & .976 & .242 & .600 \\
  & Large Single Model   & $.930 \pm .008$ & .964 & .271 & --- \\
  & Stoch.\ Retrain      & $.979 \pm .003$ & .989 & .168 & .581 \\
  & \textbf{DASH (MaxMin)} & $\mathbf{.982} \pm .003$ & \textbf{.991} &
    \textbf{.159} & \textbf{.579} \\
\bottomrule
\end{tabular}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{first_mover_concentration.pdf}
...
\midrule
\multirow{5}{*}{\rotatebox{90}{\scriptsize Dependent}}
  & Single Best          & $.960 \pm .005$ & .980 & .217 & --- \\
  & Single Best$^\dagger$ & $.962 \pm .005$ & .981 & .213 & --- \\
  & Large Single Model   & $.940 \pm .007$ & .969 & .253 & --- \\
  & LSM (Tuned)          & $.945 \pm .006$ & .972 & .245 & --- \\
  & Ensemble SHAP        & $.962 \pm .005$ & .980 & .232 & --- \\
\midrule
...
\multirow{4}{*}{\rotatebox{90}{\scriptsize Independent}}
  & Stochastic Retrain   & $.980 \pm .003$ & .989 & .172 & --- \\
  & Random Selection$^\dagger$ & $.978 \pm .003$ & .988 & .175 & --- \\
  & Naive Top-$N$        & $.976 \pm .004$ & .987 & .179 & --- \\
  & \textbf{\dash{} (MaxMin)} & $\mathbf{.981} \pm .003$ & \textbf{.990} & \textbf{.163} & --- \\
\bottomrule
\end{tabular}
\end{table}
...
The non-significant stability difference ($d = 0.26$) between \dash{}
and Stochastic Retrain is the most important result in this paper: it
confirms that model independence, not any particular pipeline design, is
...
within-group CV ($0.1625$ vs.\ $0.1719$), suggesting that forced feature
restriction distributes credit more evenly, though this difference has
not been significance-tested and should be interpreted cautiously.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{correlation_sweep.pdf}
...
\begin{tabular}{llccc}
\toprule
\textbf{Dataset} & \textbf{Method} & \textbf{Stability} & \textbf{$\Delta$Stab.} & \textbf{RMSE/AUC} \\
\midrule
...
\multirow{2}{*}{Breast Cancer}
  & Single Best            & $0.534 \pm 0.04$ & ---    & $0.97 \pm 0.01$ \\
  & \textbf{DASH (MaxMin)} & $\mathbf{0.933} \pm 0.01$ & $+0.399$ & $\mathbf{0.98} \pm 0.01$ \\
\midrule
\multirow{3}{*}{Superconductor}
  & Single Best            & $0.848 \pm 0.02$ & ---    & $9.02 \pm 0.09$ \\
  & Large Single Model     & $0.702 \pm 0.03$ & $-0.146$ & $9.17 \pm 0.08$ \\
  & \textbf{DASH (MaxMin)} & $\mathbf{0.965} \pm 0.01$ & $+0.118$ & $\mathbf{8.97} \pm 0.08$ \\
\midrule
\multirow{2}{*}{California Housing}
  & Single Best            & $0.930 \pm 0.02$ & ---    & $0.48 \pm 0.01$ \\
  & \textbf{DASH (MaxMin)} & $\mathbf{0.971} \pm 0.01$ & $+0.041$ & $\mathbf{0.47} \pm 0.01$ \\
\bottomrule
\end{tabular}\\[2pt]
...
\dash{} improves stability by $+0.041$ over Single Best while
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{ablation_sensitivity.pdf}
...
$K \leq 30$ TreeSHAP explanations. It is approximately $5$--$7\times$
more expensive than Single Best per repetition, but the cost is
embarrassingly parallel and scales linearly with compute.

\begin{table}[h]
\centering
\caption{Computational cost at $\rhovar = 0.9$ (20 repetitions).
Wall-clock times are hardware-dependent and reported for relative
comparison. Measured on a single-core CPU.}
\label{tab:cost}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{Models} & \textbf{SHAP Evals} & \textbf{Per-rep (s)} \\
\midrule
Single Best       & 30  & 1  & --- \\
Single Best ($M$=200)  & 200 & 1  & --- \\
Large Single Model & 1  & 1  & --- \\
Ensemble SHAP     & 1   & 1  & --- \\
Stochastic Retrain & 30 & 30 & --- \\
\dash{} (MaxMin)  & 200 & $K_{\text{eff}}$ & --- \\
\bottomrule
\end{tabular}
\end{table}

\noindent
Timing values (``---'') will be populated from the experimental runner
output; see \texttt{run\_experiments.py} for exact measurements.
...
This is a genuine limitation, not merely a scope boundary.
Nonlinear relationships are common in practice, and the degradation
warrants explanation: when the DGP contains interactions and nonlinear
terms, different models in the DASH ensemble may capture different
interaction structures. Averaging SHAP values across models that have
learned qualitatively different functional forms introduces noise rather
than canceling arbitrary choices. At low correlation, the features are
sufficiently distinct that single-model explanations are already stable,
and the averaging noise dominates.
...
again provides clear benefit ($+0.076$ at $\rhovar = 0.95$).
...
from $0.53$ to $0.93$ (Breast Cancer), from $0.85$ to $0.97$
(Superconductor), and from $0.93$ to $0.97$ (California Housing).
