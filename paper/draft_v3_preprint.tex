...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth,alt={Sensitivity plots of stability vs population size M, ensemble size K, epsilon threshold, and diversity threshold delta}]{ablation_sensitivity.png}
\caption{Ablation sensitivity: stability as a function of population size $M$,
ensemble size $K$, performance threshold $\varepsilon$, and diversity
threshold $\delta$ at three correlation levels.
Stability plateaus past $M = 100$ and $K = 15$;
see Table~\ref{tab:epsilon} for detailed $\varepsilon$ sensitivity.}
\label{fig:ablation}
\end{figure}
