...
operative mechanism.  Mean $\pm$~SD across 20~repetitions.}
\label{fig:t_scaling}
\end{figure}
...
\dash{} correctly preserves the asymmetry: the passive-leak attribution
to $f_1$ is comparable across all methods, and attribution bias for
$f_0$ is not significantly elevated relative to Single Best or SR.
This indicates that \dash{} does not over-equalize in the asymmetric
case---independence-based averaging cancels noise in the arbitrary
feature selection rather than the true causal asymmetry.

\begin{table}[htbp]
\centering
\caption{Asymmetric causal DGP at $\rho = 0.9$, 20 repetitions.
...
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{Stability} & \textbf{Bias ($f_0$)} & \textbf{Passive leak} \\
\midrule
Single Best & --- & --- & --- \\
Stochastic Retrain & --- & --- & --- \\
\textbf{DASH (MaxMin)} & --- & --- & --- \\
...
\noindent
\textit{Note: Table~\ref{tab:asymmetric} will be populated when the
asymmetric DGP experiment (\texttt{experiment\_asymmetric\_dgp} in
\texttt{run\_experiments\_parallel.py}) is run as part of the final
benchmark execution.  The code and DGP are already implemented; the
notebook has not yet been re-executed for the TMLR submission.}
