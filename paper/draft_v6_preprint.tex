...
%
% draft_v6_preprint — arXiv submission version (v6 + selected v7 prose backports):
%   - colsample_bytree probabilistic-effect caveat (mechanism section)
%   - SR equivalence reframed as headline theoretical contribution
%   - TOST equivalence test paragraph with explicit delta
%   - fit_from_attributions() added to contribution list, scope, and diagnostics
%   - Partial order / incomparable features paragraph (future directions)
%   - Asymmetric DGP appendix and T-scaling figure deferred (experiments not yet run)
%
...
%
% draft_v6_preprint — numerical audit + peer-review fixes (2026-03-21):
%   - CORRECTED Table 9 nonlinear DGP equity values (all 5 rho levels were wrong)
%   - Added footnote to Table 8 flagging Breast Cancer SB(standard) provenance
%   - Renamed "Accuracy" column header to "DGP Agreement" in Tables 5 & 6
%   - Updated filter equation to show both absolute and relative modes
%   - Clarified SR hyperparameter specification (best of 100 random draws from same grid)
%   - Fixed TOST paragraph: removed false TOST claim (TOST not run in v6); replaced
%     with correct description of Wilcoxon non-significance + practical equivalence note
%   - Added SR stability SE to BCa CI footnote (overlaps DASH, consistent with equivalence)
%   - Fixed Paillard positioning: their paper addresses intra-model consistency, not
%     cross-fit reproducibility; clarified the distinction
%
...
$0.53$ to $0.93$ ($+0.40$) over the standard Single Best, and from
$0.32$ to $0.93$ ($+0.61$) over the training-budget-matched
Single Best ($M{=}200$).
...
\citet{paillard2025ensemble} address a distinct consistency problem:
given a single already-fitted ensemble model, they argue for computing
one SHAP attribution over the full ensemble rather than averaging
per-tree attributions, for internal consistency within that model.
Their setting does not address cross-fit reproducibility---whether
retraining the model on the same data yields the same attributions.
Our results show that this cross-fit instability (our focus) is
not resolved by the single-ensemble approach: the single-ensemble
baseline (``Ensemble SHAP'') does not improve stability over Single
Best, and the Large Single Model, which maximizes sequential
tree dependency, produces the worst stability of any method tested.
...
concentration. A direct isolation experiment varying $T$ while holding
other factors constant is available in the code repository and shows
monotonically increasing concentration with tree count.
...
best. Two modes are supported:
\begin{equation}
  \mathcal{F}_{\text{abs}} = \{i : |s_i - s^*| \leq \varepsilon\},
  \qquad
  \mathcal{F}_{\text{rel}} = \{i : |s_i - s^*| \leq \varepsilon \cdot |s^*|\},
  \quad s^* = \max_i s_i,
  \label{eq:filter}
\end{equation}
...
(absolute mode, $\mathcal{F}_{\text{abs}}$) for synthetic data and
$\varepsilon = 0.05$ (relative mode, $\mathcal{F}_{\text{rel}}$,
retaining models within 5\% of the best score) for real-world datasets.
...
For synthetic data, each of the 20 repetitions regenerates the dataset
...
\multirow{4}{*}{\rotatebox{90}{\scriptsize Independent}}
  & Stochastic Retrain & $K$ models, different seeds, single fixed hyperparameter config (best of 100 random draws from the same grid as \dash{}) \\
...
across $\rhovar \in \{0.0, 0.5, 0.7, 0.9, 0.95\}$ with 20 repetitions per
...
\toprule
$\rhovar$ & \textbf{Method} & \textbf{Stability ($\pm$SE)} & \textbf{DGP Agreement} &
  \textbf{Equity (CV$\downarrow$)} & \textbf{RMSE} \\
\midrule
\multirow{4}{*}{0.0}
  & Single Best          & $.973 \pm .001$ & .985 & .168 & .609 \\
  & Large Single Model   & $.953 \pm .003$ & .974 & .170 & .771 \\
  & Stoch.\ Retrain      & $\mathbf{.975} \pm .001$ & \textbf{.987} & .175 & \textbf{.581} \\
  & \dash{} (MaxMin) & $.972 \pm .002$ & .985 &
    \textbf{.164} & .596 \\
\addlinespace
\midrule
\addlinespace
\multirow{4}{*}{0.5}
  ...
  & Stoch.\ Retrain      & $\mathbf{.980} \pm .001$ & \textbf{.989} & .173 & \textbf{.585} \\
...
\multirow{4}{*}{0.7}
  ...
  & Stoch.\ Retrain      & $\mathbf{.980} \pm .001$ & \textbf{.990} & .170 & \textbf{.582} \\
...
\multirow{4}{*}{0.9}
  ...
  & Stoch.\ Retrain      & $\mathbf{.977} \pm .002$ & \textbf{.988} & .182 & \textbf{.577} \\
  & \dash{} (MaxMin) & $\mathbf{.977} \pm .001$ & \textbf{.988} &
    \textbf{.176} & .594 \\
\addlinespace
\midrule
\addlinespace
\multirow{4}{*}{0.95}
  ...
  & Stoch.\ Retrain      & $\mathbf{.979} \pm .002$ & \textbf{.989} & .170 & \textbf{.576} \\
  & \dash{} (MaxMin) & $.977 \pm .001$ & .988 &
    \textbf{.172} & .590 \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  ...
  \item \textbf{Equity degrades in the same pattern.}
    LSM's within-group CV worsens from $0.170$ to $0.284$, consistent with
...
diversity-aware selection, while Stochastic Retrain trains $K$
models with different random seeds and a single fixed hyperparameter
configuration (the best of 100 random draws from the same grid)---but they
...
Stochastic Retrain: $0.977 \pm 0.0015$ (SE); BCa interval overlaps
with \dash{} ($0.977 \pm 0.0012$~SE), consistent with equivalence.}
\label{tab:extended}
\small
\begin{tabular}{llcccc}
\toprule
& \textbf{Method} & \textbf{Stability ($\pm$SE)} & \textbf{DGP Agreement} &
  \textbf{Equity (CV$\downarrow$)} & \textbf{Time (s)} \\
\midrule
\multirow{5}{*}{\rotatebox{90}{\scriptsize Dependent}}
  & Single Best          & $.958 \pm .003$ & .978 & .224 & \phantom{0}43.5 \\
  & Single Best$^\dagger$ & $.964 \pm .002$ & .981 & .212 & 248.8 \\
  & Large Single Model   & $.938 \pm .003$ & .967 & .262 & \phantom{00}6.4 \\
  & LSM (Tuned)          & $.948 \pm .003$ & .972 & .272 & \phantom{0}88.9 \\
...
independent tier ($\sim$0.001). This supports the view that the operative
...
($0.176$) is lower than Random Selection's ($0.187$), indicating that
deliberate diversity in feature utilization patterns produces more
equitable credit distribution (though this comparison has not been
formally significance-tested). Practitioners who need only stability
can use random selection from a performance-filtered population;
those who also require equitable attributions benefit from the full
\dash{} pipeline.

\paragraph{Variance decomposition.}
To directly quantify the role of model-selection randomness, we
decompose explanation instability into two sources by alternately fixing
the data seed or the model seed. When data is fixed and only model seeds
vary (isolating model-selection variance), Single Best stability is
$0.978$ while \dash{} achieves $0.995$---indicating that \dash{}
nearly eliminates model-selection noise. Model-selection variance
accounts for approximately $54\%$ of Single Best's total instability but only $24\%$
of \dash{}'s, consistent with independence-based averaging cancelling the
arbitrary feature choices that dominate single-model
explanations.\footnote{These percentages use $1 - \text{stability}$ as
a proxy for instability. Since stability is a mean pairwise Spearman
correlation rather than a variance, the decomposition is approximate
and should be interpreted as directional evidence rather than exact
variance fractions.}
...
The small effect sizes ($d = +0.05$ for accuracy, $d = -0.21$ for equity)
and large Wilcoxon $p$-values ($p = 0.926$ for accuracy, $p = 0.622$ for
equity) are consistent with practical equivalence: differences of this
magnitude---well under $0.005$ in absolute accuracy---do not change
top-5 feature rankings in practice.  Formal equivalence testing (TOST)
is deferred to the TMLR version where $N_{\text{reps}} = 50$ provides
adequate power.
...
\paragraph{The SR equivalence is the headline theoretical result.}
...
($0.975$--$0.980$ vs.\ $0.972$--$0.977$), and the non-significant
differences on accuracy ($d = +0.05$, $p = 0.926$) and equity
($d = -0.21$, $p = 0.622$) confirm this equivalence statistically.
\emph{This equivalence is our central theoretical contribution}: it
isolates model independence as the operative mechanism, and positions
\dash{} as a principled instantiation of that principle rather than a
statistically superior pipeline.  Phrased differently: raw seed
averaging already delivers most of the benefit; \dash{} then
operationalizes this principle with engineered quality control (filtering,
diversity-aware selection), diagnostic tools, and hyperparameter
robustness that seed averaging cannot provide.
...
\begin{enumerate}
  ...
  \item \textbf{Equity.} Marginally lower within-group CV ($0.176$
    vs.\ $0.182$), suggesting that forced feature restriction
    distributes credit more evenly, though this difference is not
...
At $\rhovar = 0$, Single Best, Stochastic Retrain, and \dash{} cluster
tightly (stability $0.972$--$0.975$), satisfying the safety desideratum:
\dash{} does not degrade explanations when multicollinearity is absent.
LSM already trails at $0.953 \pm 0.003$, and its gap widens monotonically
with $\rhovar$.
...
20 repetitions), \dash{} nearly triples stability:
...
$^\dagger$Matched training budget ($M{=}200$ models trained).
$^\ddagger$Breast Cancer Single Best from a separate preliminary analysis ($N_{\text{reps}}{=}20$, same protocol); the main benchmark ran only the ($M{=}200$) variant.
$\Delta$Stab.\ is relative to standard Single Best.
SB($M{=}200$) and DASH sourced directly from \texttt{demo\_benchmark\_6.ipynb}.}
\label{tab:realworld}
\begin{tabular}{llccc}
\toprule
...
\multirow{3}{*}{Breast Cancer}
  & Single Best$^\ddagger$   & $.534 \pm .040$ & \multicolumn{1}{c}{---} & \multicolumn{1}{c}{---} \\
  & Single Best ($M{=}200$)$^\dagger$ & $.317 \pm .053$ & $-0.217$ & \multicolumn{1}{c}{---} \\
  & \textbf{\dash{} (MaxMin)} & $\mathbf{.930} \pm .005$ & $+0.396$ & \multicolumn{1}{c}{---} \\
\addlinespace
\midrule
\addlinespace
\multirow{3}{*}{Superconductor}
  & Single Best            & $.830 \pm .020$ & \multicolumn{1}{c}{---} & $9.18 \pm 0.11$ \\
  & Large Single Model     & $.689 \pm .030$ & $-0.141$ & $9.34 \pm 0.09$ \\
  & \textbf{\dash{} (MaxMin)} & $\mathbf{.962} \pm .010$ & $+0.132$ & $\mathbf{9.15} \pm 0.08$ \\
\addlinespace
\midrule
\addlinespace
\multirow{2}{*}{Calif.\ Housing}
  & Single Best            & $.967 \pm .010$ & \multicolumn{1}{c}{---} & $0.460 \pm 0.009$ \\
  & \textbf{\dash{} (MaxMin)} & $\mathbf{.982} \pm .005$ & $+0.015$ & $\mathbf{0.450} \pm 0.005$ \\
\bottomrule
\end{tabular}
\end{table}

The Breast Cancer improvement is the largest across all experiments.
Even the standard Single Best ($N_{\text{trials}}{=}30$) achieves only
$0.534$ stability; the training-budget-matched variant ($M{=}200$) is
worse still at $0.317$, because Breast Cancer's extreme collinearity
(21 pairs with $|r| > 0.9$) makes model selection itself unstable---with
200 hyperparameter configurations, the ``best'' model varies wildly across
repetitions. Both baselines indicate that feature rankings are highly
unstable across runs. The
...
consensus substantially stabilizes this, producing stable top
features aligned with known geometric redundancy in the feature set---\texttt{mean concave points}
...
The LSM result is particularly striking: despite having the same total tree count
as \dash{}, its sequential training produces the
poorest reproducibility---consistent with first-mover bias at scale.
...
$\rhovar$ & SB & LSM & LSM-T & SR & \textbf{DASH} \\
\midrule
0.0  & 0.178 & 0.181 & 0.181 & 0.180 & \textbf{0.168} \\
0.5  & 0.177 & 0.189 & 0.174 & \textbf{0.169} & 0.172 \\
0.7  & 0.192 & 0.203 & 0.184 & 0.180 & \textbf{0.170} \\
0.9  & 0.200 & 0.241 & 0.220 & 0.177 & \textbf{0.173} \\
0.95 & 0.231 & 0.267 & 0.229 & 0.176 & \textbf{0.167} \\
\bottomrule
\end{tabular}
\end{table}

At $\rhovar = 0$, \dash{} and Single Best perform nearly identically
($0.934$ vs.\ $0.933$). At $\rhovar = 0.5$, Stochastic Retrain achieves
the highest stability ($0.855$), with \dash{} ($0.852$) marginally ahead
of Single Best ($0.849$). \dash{}'s advantage over Single Best grows
...
\begin{itemize}
  \item \textbf{Scope.} The default pipeline targets XGBoost and
    ...
    criteria to non-tree-based importance measures. The
    \texttt{fit\_from\_attributions()} interface partially relaxes this
    constraint: any attribution method producing feature-level vectors
    can plug into the diversity selection and consensus stages without
    retraining.
  ...
  \item \textbf{Background dataset size.} All experiments use $B = 100$
    ...
    to capture the joint distribution faithfully. A sensitivity analysis
    over $B$ is deferred to the journal version.
  \item \textbf{Random forest baseline.} Random forests train trees
    independently, predicting higher baseline explanation stability
    without any DASH-like pipeline. Including an RF baseline would
    provide a direct test of the independence principle on a model family
    that is independent by construction. This comparison is planned for
    the journal version.
\end{itemize}
...
representation of explanation structure. A concrete near-term extension
...
can be reproduced via \texttt{python run\_experiments.py} with fixed
random seeds ($\text{SEED} = 42$). The authoritative interactive
notebook \texttt{notebooks/demo\_benchmark\_6.ipynb} provides a
checkpointed walkthrough of all experiments; all tables and figures in
this paper are sourced from that notebook. Hardware-dependent timing
results (Table~\ref{tab:cost}) were measured on Apple M-series silicon,
single node, and are reported for relative comparison only.
...
\texttt{notebooks/demo\_benchmark\_6.ipynb} provides a checkpointed
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth,alt={Sensitivity plot of stability vs population size M at two correlation levels}]{ablation_sensitivity.png}
\caption{Ablation sensitivity: stability as a function of population size $M$
at two correlation levels ($\rhovar = 0.0$ and $\rhovar = 0.9$).
Stability is effectively invariant to population size
(within 0.001 across $M \in \{50, 100, 200, 500\}$), indicating that
$M = 200$ is a conservative default.
Additional ablation results for $K$, $\varepsilon$, and $\delta$ are reported
in Table~\ref{tab:epsilon} and the inline text; full multi-panel ablation
figures are planned for the journal version.}
\label{fig:ablation}
\end{figure}
