...
%
% draft_v6_preprint — revised per ArXiv readiness review:
%   - Changed "pre-registered" to "pre-specified" (Appendix C)
%   - Added variance decomposition approximate-decomposition caveat
...
%   - Added arXiv subject classification
...
%   - Consistent "tree-count-matched" terminology throughout (abstract + body)
...
$= 0.977$, numerically identical, while the standard
...
$0.32$ to $0.93$ ($+0.61$), comparing against a tree-count-matched
...
\smallskip
\noindent\textbf{arXiv subject:} cs.LG (Machine Learning); stat.ML (Statistics -- Machine Learning)
...
\multirow{5}{*}{\rotatebox{90}{\scriptsize Dependent}}
  ...
  & Single Best ($M$) & Best of $M{=}200$ models (tree-count-matched to \dash{}) \\
...
substantially (Table~\ref{tab:cost}): ``tree-count-matched'' refers to
the total number of trees, not wall-clock time or FLOPs. The comparison isolates the
...
$\rhovar = 0.9$ in Table~\ref{tab:extended}. Ensemble SHAP, Single Best ($M{=}200$),
and the remaining baselines are evaluated at $\rhovar = 0.9$ only (Table~\ref{tab:extended})
rather than across all $\rhovar$ levels, because their computational cost
scales with the number of sweep points and their primary purpose is to
contextualize the mechanism at the critical high-correlation regime.}
\label{tab:sweep}
\small
...
$^\dagger$Matched tree-count budget ($M{=}200$ models trained).
...
equitable credit distribution. Practitioners who need only stability
...
arbitrary feature choices that dominate single-model explanations.
(Caveat: these percentages use $1 - \text{stability}$ as a proxy for
instability. Since stability is a mean pairwise Spearman correlation
rather than a variance, the decomposition is approximate and the
model-selection and data-sampling components do not satisfy an exact
additive partition. The figures should be interpreted as directional
evidence of relative contribution, not as precise variance fractions.)
...
\multirow{2}{*}{Breast Cancer}
  & Single Best ($M{=}200$) & $.317 \pm .053$ & \multicolumn{1}{c}{---} & \multicolumn{1}{c}{---} \\
  & \textbf{\dash{} (MaxMin)} & $\mathbf{.930} \pm .005$ & $+0.614$ & \multicolumn{1}{c}{---} \\
\addlinespace
\midrule
\addlinespace
...
experiments. We report the tree-count-matched Single Best ($M{=}200$) for
Breast Cancer rather than the standard Single Best ($N_{\text{trials}}{=}30$)
used for Superconductor and California Housing, because Breast Cancer's
extreme collinearity (21 pairs with $|r| > 0.9$) makes the compute-matched
comparison the more stringent test: even with 200 hyperparameter trials,
...
($0.934$ vs.\ $0.933$). At $\rhovar = 0.5$, \dash{} shows a marginal
advantage ($0.852$ vs.\ $0.849$), which grows consistently with
...
Stability plateaus past $M = 100$, confirming that $M = 200$ is sufficient.
