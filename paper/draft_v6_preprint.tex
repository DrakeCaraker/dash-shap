...
% ===========================================================================
\begin{abstract}
We isolate and empirically characterize \emph{first-mover bias}---a
path-dependent concentration of feature importance caused by sequential
residual fitting in gradient boosting---as a specific mechanistic cause of the
well-known instability of SHAP-based feature rankings under
multicollinearity. When correlated features
...
the same total tree count as our method produces the \emph{worst} explanations
of any approach tested.

We demonstrate that \emph{model independence} is sufficient to resolve
first-mover bias in the linear regime, and remains the most effective
mitigation under nonlinear data-generating processes. Both our proposed
...
chain, confirming that the operative mechanism is independence between
...
XGBoost model with thousands of trees and the same total tree count as \dash{} ($M \times {\sim}75 \approx 15{,}000$ trees, where ${\sim}75$ is the average number of trees per population model after early stopping) produces the \emph{worst} explanations of any method
...
\begin{itemize}
  \item \textbf{Phenomenon.} Feature selection bias in ensemble methods
    is well known \citep{strobl2007bias, hooker2019please}; we isolate
    and empirically characterize the specific mechanistic
    pathway---sequential residual dependency in gradient
    boosting---through which it concentrates SHAP-based feature
    attributions on arbitrary features under collinearity.
  \item \textbf{Principle.} We demonstrate that model independence is
    sufficient to resolve first-mover bias in the linear regime, and
    remains the most effective mitigation under nonlinear
    data-generating processes.  Both \dash{} (deliberate diversity) and
    Stochastic Retrain (seed diversity) restore stability to the same
    level, confirming that the operative mechanism is independence
...
produces the worst explanations of any method, consistent with the
...
other factors constant is available in the code repository and confirms
...
results should be understood as confirming that the pipeline achieves
...
cost parity. The only structural difference is that \dash{}'s models
...
purpose is to contextualize the mechanism at high correlation. The key finding is that LSM achieves the \emph{worst}
stability of any method at every correlation level, despite matching
\dash{}'s total tree count and feature restriction. This confirms
...
of \dash{}'s, confirming that independence-based averaging cancels the
...
the operative mechanism. Indeed, SR achieves marginally higher stability
point estimates than \dash{} at most $\rhovar$ levels ($0.975$--$0.980$
vs.\ $0.972$--$0.977$), though these differences are small
($\leq 0.003$) and not statistically significant.
\dash{} should therefore be understood as a
...
advantages over SR are threefold:
\begin{enumerate}
...
tightly (stability $0.972$--$0.975$), confirming the safety desideratum:
...
All results sourced from \texttt{demo\_benchmark\_6.ipynb}.}
\label{tab:realworld}
\begin{tabular}{llccc}
\toprule
...
\multirow{3}{*}{Breast Cancer}
  & Single Best              & $.534 \pm .040$ & \multicolumn{1}{c}{---} & \multicolumn{1}{c}{---} \\
...
repetitions. Both baselines indicate that feature rankings are essentially
random across runs. The
...
Bold indicates best per $\rhovar$. Equity (CV) values are reported in the lower section.}
\label{tab:nonlinear}
...
not fully restore stability (Section~\ref{sec:robustness}), as different
models may capture qualitatively different interaction structures whose
averaging introduces noise alongside the beneficial cancellation of
arbitrary feature selections.
...
\begin{enumerate}
  \item The Large Single Model, which maximizes sequential dependency,
    ...
    total tree count and feature restriction. This confirms that
...
(within 0.001 across $M \in \{50, 100, 200, 500\}$), confirming that
...
\noindent\textit{Note:} Random Selection is slower than \dash{} despite
a similar pipeline (both train $M{=}200$ models and compute SHAP for
$K_{\text{eff}}$ models) because \dash{}'s diversity selection identifies
a compact subset early, whereas Random Selection's lack of diversity
guarantees leads to higher variance in per-run $K_{\text{eff}}$ and
additional overhead from the random sampling procedure across the full
filtered pool.
\end{table}
