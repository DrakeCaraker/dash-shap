...
% --- TMLR style ---
% For submission (anonymous): \usepackage{tmlr}
% For preprint (de-anonymized): \usepackage[preprint]{tmlr}
...
\let\AND\relax  % resolve conflict between tmlr.sty and algorithmic
...
\title{First-Mover Bias in Gradient Boosting Explanations: Mechanism, Detection, and Resolution}

% Authors hidden for anonymous review.
% De-anonymize by switching to \usepackage[preprint]{tmlr} above.
\author{\name Drake Caraker \email drakecaraker@gmail.com \\
      \addr Independent Researcher \\
      ORCID: 0009-0009-5639-7899
      \AND
      \name Bryan Arnold \email puremath86@gmail.com \\
      \addr Independent Researcher \\
      ORCID: 0009-0007-8589-8989
      \AND
      \name David Rhoads \email drhoads9@gmail.com \\
      \addr Independent Researcher \\
...
\url{https://anonymous.4open.science/r/dash-shap-FFEC}.
\end{abstract}
...
\citet{caraker2026impossibility} formalizes a
...
vs.\ hyperparameter diversity).  \citet{caraker2026impossibility}
prove this averaging is the unique
...
\citet{caraker2026impossibility} proves that
...
\citet{caraker2026impossibility} introduces
...
The diversity threshold $\delta$ is the one sensitive parameter:
stability degrades from $0.977$ to $0.963$ at $\delta = 0.20$, with
the default $\delta = 0.05$ at the elbow
...
Notably, \dash{} significantly outperforms Stochastic Retrain under
nonlinearity at $\rhovar \geq 0.9$ ($0.887$ vs.\ $0.857$; bootstrap
95\% CIs non-overlapping), in contrast to their equivalence in the
linear regime---indicating that hyperparameter diversity, not just seed
diversity, matters when models learn qualitatively different functional
forms.
...
The impossibility analysis of \citet{caraker2026impossibility}
...
collinearity. \citet{caraker2026impossibility} formalize the conditions
under which partial orders are necessary rather than merely convenient.
...
\begin{itemize}
  \item \textbf{Scope of the independence principle.} The empirical
    ...
    The impossibility theorem of
    \citet{caraker2026impossibility} proves that the underlying
    ...
    reference distribution \citep{aas2021explaining},
    \citet{caraker2026impossibility} prove that switching to
    conditional SHAP does \emph{not} escape the impossibility when
    features have equal causal effects---the instability is a property of
  ...
  \item \textbf{Background dataset size.} All experiments use $B = 100$
    background samples for interventional TreeSHAP. A sensitivity analysis
    across $B \in \{50, 100, 200, 500\}$ at $\rhovar = 0.9$ shows
    stability variation below $0.0002$, suggesting this choice is not
    critical for the current datasets; however, for high-dimensional
    datasets with strong correlation structure, larger $B$ may be needed
...
$\rhovar$---is consistent with the formal impossibility result of
\citet{caraker2026impossibility}: no
...
This incompatibility holds for any attribution method applied to any model
class exhibiting the Rashomon property under correlation---not only
gradient boosting.  Indeed, the companion paper proves a stronger
\emph{bilemma}: even for binary sign attribution (merely determining
whether a feature's effect is positive or negative), faithfulness and
stability cannot coexist---completeness is not required to generate the
impossibility.\footnote{The impossibility is not fragile:
\citet{caraker2026impossibility} prove an approximate bilemma showing
that faithfulness--stability incompatibility persists at every
$\varepsilon$-tolerance level, and a quantitative version bounding
$\text{unfaith}_1 + \text{unfaith}_2 \geq \Delta - \delta$.}
The present work provides the empirical foundation
and constructive resolution; the theorem of \citet{caraker2026impossibility},
mechanically verified in the Lean~4 proof assistant, proves that ensemble-based
approaches like \dash{} are not merely effective but mathematically
necessary for achieving both stability and equity.
The resolution taxonomy of the companion paper further shows that
\emph{collapsed tightness}---the specific form of incompatibility
governing binary sign attribution---can only be resolved by
\emph{enrichment}: expanding the output space to include a neutral
element that permits ties.  \dash{}'s consensus averaging implements
precisely this enrichment, placing it in the unique structural
resolution class for the attribution impossibility.
...
\url{https://anonymous.4open.science/r/dash-shap-FFEC}. The complete benchmark
...
% ===========================================================================
% Bibliography is inline (thebibliography).
% TMLR uses tmlr.bst but thebibliography is also accepted.
...
\bibitem[Anonymous(2026b)]{caraker2026impossibility}
Anonymous.
\newblock The attribution impossibility: Faithful, stable, and complete
  feature rankings cannot coexist under collinearity.
\newblock \emph{arXiv preprint}, 2026. Lean~4 formalization available at
  \url{https://anonymous.4open.science/r/dash-impossibility-lean-790D}.
...
available at \url{https://anonymous.4open.science/r/dash-shap-FFEC}.
...
case: two-step linear boosting on correlated features.
\citet{caraker2026impossibility} prove the formal result---the
