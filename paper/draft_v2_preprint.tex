...
\paragraph{Informal claim.}
Let $f = \sum_{t=1}^{T} h_t$ be a gradient-boosted model with $T$
trees, and let features $j, k$ belong to a correlated group with
pairwise correlation $\rhovar \to 1$. If tree $h_1$ splits on feature
$j$, then $\mathbb{E}[|\phi_j(f)|] > \mathbb{E}[|\phi_k(f)|]$ under
TreeSHAP, with the gap increasing in $T$. The expectation is over the
randomness in data sampling and split selection. This claim is supported
empirically in Section~\ref{sec:mechanism_result}; a formal proof is
...
substantially (Table~\ref{tab:cost}): ``compute-matched'' refers to
total trees, not wall-clock time. The comparison isolates the
...
\begin{figure}[htbp]
\centering
% NOTE: Full 4-panel ablation figure (M, K, epsilon, delta) to be
% regenerated from demo_benchmark_7 once that run completes.
\includegraphics[width=\textwidth,alt={Sensitivity plot of stability vs population size M at three correlation levels}]{ablation_sensitivity.png}
\caption{Ablation sensitivity: stability as a function of population size $M$
at three correlation levels (remaining panels---$K$, $\varepsilon$,
$\delta$---to be updated in the final version).
Stability plateaus past $M = 100$;
see Table~\ref{tab:epsilon} for $\varepsilon$ sensitivity.}
\label{fig:ablation}
\end{figure}
