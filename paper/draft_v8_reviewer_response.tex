...
Nonetheless, the cleanest isolation would test Stochastic Retrain with
\emph{unrestricted} \texttt{colsample\_bytree} ($0.6$--$1.0$), preserving
model independence while removing the forced feature restriction that
both \dash{} and standard SR share (see footnote on
Table~\ref{tab:baselines}).  This experiment is deferred to future work.
Until such evidence is available, our central claim should be read as:
model independence \emph{combined with} SHAP averaging is the operative
mechanism, with the Ensemble SHAP counter-example providing the strongest
available evidence that independence is the dominant factor.
