...
%
% draft_v8_reviewer_response — addresses adversarial peer review (v8 changes):
%   - Softened MVUE/Cramér-Rao claims to cite companion work (#3)
%   - Documented SR hyperparameter selection procedure (#6)
%   - Noted equity reversal at rho=0.95 (#5)
%   - Reframed TOST margin as practical significance (#7)
%   - Added IS Plot threshold sensitivity and FSI denominator notes (#14, #15)
%   - Added bootstrap exchangeability assumption note (#19)
%   - Added averaging-vs-independence confound paragraph in Discussion (#1)
%   - Added positioning vs Fisher/Bilodeau/Chen paragraph (#9)
%   - Added causal semantics clarification in Limitations (#8)
%   - Added practitioner decision rules and memory footprint guidance (#16, #17)
%   - Expanded LightGBM/CatBoost limitation (#10)
%   - Added column-ordering bias limitation (#20)
%   - Added two-tree analytical model limitations paragraph (#2)
%   - Added success criteria self-critique (#11)
%   - Added LIME validation limitations paragraph (#18)
%   - Added library version pins and cross-platform note (#12, #13)
%   - Added bilemma/sign-stability cross-references from companion paper
%   - Added Rashomon unfaithfulness bound quantification
%   - Added sign stability limitation (ranking vs sign scope)
%
...
trained models---is established in companion work as the minimum-variance
unbiased estimator (Cram\'er--Rao bound) and is shown to be Pareto-optimal
among all stable attribution methods \citep{caraker2026impossibility}.  This core
...
\multirow{4}{*}{\rotatebox{90}{\scriptsize Independent}}
  & Stochastic Retrain\footnote{SR selects its fixed configuration by
    training 100 random configurations from the same restricted search
    space as \dash{} (\texttt{colsample\_bytree}
    $\in \{0.1, 0.15, \ldots, 0.5\}$, \texttt{max\_depth}
    $\in \{3,\ldots,12\}$, etc.), choosing the best by validation score,
    then retraining $K$ models with different random seeds using that
    configuration.  Both SR and \dash{} thus share the low
    \texttt{colsample\_bytree} restriction; isolating independence from
    feature restriction would require testing SR with unrestricted
    \texttt{colsample\_bytree} (see Discussion).}
  & $K$ models, different seeds, same hyperparameters \\
...
\multirow{4}{*}{0.9}
  ...
  & Stoch.\ Retrain      & $\mathbf{.978} \pm .001$ & \textbf{.988} & .180 & \textbf{.576} \\
  & \dash{} (MaxMin) & $.977 \pm .001$ & .988 &
    \textbf{.175} & .591 \\
\addlinespace
\midrule
...
\addlinespace
\multirow{4}{*}{\rotatebox{90}{\scriptsize Independent}}
  & Stochastic Retrain   & $.977 \pm .002$ & .988 & .180 & 30 & 233.5 \\
  & Random Selection$^\dagger$ & $.976 \pm .001$ & .988 & .186 & 30 & 287.2 \\
  & Naive Top-$N$        & $.976 \pm .001$ & .988 & .189 & 30 & --- \\
  & \textbf{\dash{} (MaxMin)} & $\mathbf{.977} \pm .001$ & \textbf{.988} & \textbf{.175} & $\sim$12 & 140.3 \\
\bottomrule
\end{tabular}
\end{table}
...
($0.175$) is significantly lower than Random Selection's ($0.186$;
...
and only model seeds vary, Single Best stability is $0.976$ while
...
$\delta \approx 0.049$.  We justify this margin on practical grounds: a 5\% relative difference
in importance accuracy represents the approximate threshold below which
a practitioner would not change feature selection decisions---at
$\rhovar = 0.9$, accuracy differences smaller than $0.049$ produce
identical top-5 feature rankings in over 95\% of repetitions,
supporting this calibration.  We note that $\delta$ is data-derived
(a function of the pooled metric mean) rather than pre-specified; a
fixed margin of $\delta = 0.02$ would be a more conservative alternative.  TOST confirms equivalence on the California Housing
...
The bootstrap assumes exchangeability of repetitions---that is, that
each repetition's importance vector is drawn from the same
distribution.  This assumption is satisfied by construction (each
repetition uses an independent random seed with the same DGP and
sample size) but would be violated under serial dependence (\eg
adaptive seed selection conditioned on previous repetitions' results).
...
\citep{caraker2026impossibility} establishes that this averaging is the unique
...
The companion paper \citep{caraker2026impossibility} establishes that
...
\begin{enumerate}
  ...
  \item \textbf{Equity.} Significantly lower within-group CV than
    Random Selection ($0.175$ vs.\ $0.186$; $p < 0.001$, $d = -0.37$),
    ...
    smaller and not significant ($d = -0.13$, $p = 0.44$) at
    $\rhovar = 0.9$.  At $\rhovar = 0.95$, SR achieves lower
    within-group CV than \dash{} ($0.159$ vs.\ $0.171$), reversing the
    direction observed at lower $\rhovar$ levels; the difference remains
    non-significant.
\end{enumerate}
...
from $0.170$ to $0.277$ ($+67\%$), while \dash{} ranges from $0.153$ to
$0.175$ ($+7\%$). This is consistent with sequential residual dependency
...
\noindent\emph{Sensitivity note.}  Quadrant boundaries are set at the
median importance and median FSI of the feature set, making them
data-dependent; features near the boundary may shift quadrants under
different subsamples or alternative threshold choices (\eg elbow
detection or bootstrap-derived thresholds).  We recommend treating
quadrant assignments as approximate and using the continuous FSI values
for fine-grained reliability assessment.  Additionally, FSI (a
coefficient of variation) is most interpretable for features with
non-trivial importance (Quadrants~I and~II); features in Quadrant~IV
may exhibit high FSI simply because the denominator (mean importance)
is near zero, and should be interpreted as noisy low-signal features
rather than genuinely unstable attributions.
...
0.5  & 0.850 & 0.836 & 0.846 & \textbf{0.860} & 0.857 \\
...
$\rhovar$ & SB & LSM & LSM-T & SR & \textbf{DASH} \\
\midrule
0.0  & 0.174 & 0.178 & 0.178 & 0.177 & \textbf{0.163} \\
...
\paragraph{Isolating independence from averaging.}
A methodological limitation of our experimental design is that we cannot
fully disentangle the effect of \emph{averaging} $K$ estimates from the
effect of \emph{independence} among those estimates.  Any single
XGBoost model's SHAP values are already implicitly aggregated across
its constituent trees by TreeSHAP---a 2000-tree model's attributions
reflect contributions from all 2000 sequentially dependent trees.
Ensemble SHAP (Table~\ref{tab:extended}) effectively ``averages'' across
these dependent trees yet achieves only $0.956$ stability, well below the
independent-tier methods ($\approx 0.977$).  This between-tier gap
provides the strongest available evidence that independence, not merely
the number of trees or the act of aggregation, is the operative factor.
A colsample ablation (Appendix~\ref{app:ablation},
\S\ref{par:colsample_ablation}) provides direct evidence: \dash{} and
Random Selection with unrestricted \texttt{colsample\_bytree}
($0.5$--$1.0$) achieve only Single-Best-level stability ($0.953$ vs.\
$0.958$), while the same methods with low \texttt{colsample\_bytree}
($0.1$--$0.5$) achieve $0.976$--$0.977$ ($p < 0.001$).  At
$\rhovar = 0$ the colsample range has no effect ($p = 0.54$), confirming
that forced feature restriction is specifically needed under collinearity.
The one remaining gap is a direct test of Stochastic Retrain with
unrestricted \texttt{colsample\_bytree}; until such evidence is
available, our central claim should be read as: model independence
\emph{combined with} forced feature restriction is the operative
mechanism, with the colsample ablation showing that restriction alone
accounts for a $+0.023$ stability gap and the Ensemble SHAP
counter-example showing that aggregation without independence is
insufficient.

\paragraph{Positioning relative to prior work.}
Our contribution is complementary to, not competitive with, the
Rashomon-set literature.  \citet{fisher2019all} characterize the
\emph{range} of variable importance across near-optimal models; we show
how to \emph{aggregate} across that range to produce stable point
estimates.  \citet{bilodeau2024impossibility} prove that no
single-model attribution method can reliably identify feature effects
across sufficiently rich model classes; our independence principle is a
constructive response---by averaging across independently trained
models, we circumvent the single-model impossibility.
The companion work further quantifies the cost of stability: any
stable attribution method is unfaithful to at least one model in every
Rashomon pair, implying a worst-case unfaithfulness rate of
$\geq$50\% for binary sign attribution on Rashomon-connected features.
\citet{chenrashomongb2024} provide an information-theoretic framework
for gradient-boosting Rashomon sets; our first-mover bias hypothesis
offers a complementary \emph{mechanistic} account specific to SHAP
attributions.  In each case, the prior work establishes the
\emph{problem} (importance instability is fundamental); we contribute a
\emph{resolution} (model-independent averaging) and \emph{diagnostics}
(FSI, IS~Plot) for detecting when the problem affects a particular
analysis.

\paragraph{Quantitative bridge via mutual information.}
The companion work also establishes a quantitative chain linking feature
dependence to explanation error: if two features share positive mutual
information ($I > 0$), there exist Rashomon-equivalent models whose
attributions for those features have opposite signs; the triangle
inequality then implies that any stable explanation has error
$\geq \Delta/2$ on at least one witness, where $\Delta$ is the
attribution gap between the models.  This floor is unreachable---no
algorithm can beat it.  Mutual information thus serves as the
\emph{exact boundary} for the impossibility: $I > 0$ triggers it,
$I = 0$ does not.  Standard correlation-based diagnostics (Pearson
$\rhovar$, VIF) miss nonlinear dependencies entirely (e.g., $X_2 =
X_1^2$ yields $|\rhovar| \approx 0$, VIF $\approx 1$, but $I = 1.91$).
On the Breast Cancer dataset, MI-based pre-screening identifies 292 of
435 feature pairs as structurally dependent despite low linear
correlation, each subject to the $\Delta/2$ error floor.
\dash{} resolves this by changing the question: instead of producing a
single ranking (which must violate either faithfulness or stability when
$I > 0$), it aggregates across models to produce a stable consensus
at the cost of decisive pairwise ordering---the enrichment that the
companion paper proves is the \emph{only} structural escape from
collapsed tightness.
...
(1)~if feature correlations are low ($\rhovar < 0.5$) and the dataset
is moderate-sized, a single well-tuned model's SHAP values are likely
adequate---our results show negligible stability differences in this
regime;
(2)~if moderate correlations exist ($0.5 \leq \rhovar < 0.8$) and
stability is the primary concern, Stochastic Retrain (seed averaging)
provides the simplest solution with minimal implementation effort;
(3)~if correlations are high ($\rhovar \geq 0.8$), equitable credit
distribution within correlated groups matters, or ground-truth-free
diagnostics are needed, use the full \dash{} pipeline.
In all cases, the FSI and IS~Plot can serve as a preliminary
collinearity audit regardless of which explanation method is ultimately
adopted.

\paragraph{Memory and compute.}
\dash{}'s peak memory footprint scales as $O(M \times S_{\text{model}})$
during the population stage, where $S_{\text{model}}$ is the in-memory
size of a single XGBoost model (typically 1--10\,MB for models with
moderate depth and early stopping).  At $M = 200$ this can require
200\,MB--2\,GB for the model population; the SHAP computation stage
adds $O(K \times N' \times P)$ for the attribution matrices.  For
wall-clock timing, see Table~\ref{tab:cost}.
...
\begin{itemize}
  \item \textbf{Scope of the independence principle.} The empirical
    ...
    networks with KernelSHAP) remains future work.  The effect pattern
    may differ for alternative GBDT implementations: CatBoost's ordered
    boosting randomizes the training-example ordering rather than the
    feature ordering, potentially reducing first-mover bias through a
    different mechanism; LightGBM's histogram-based leaf-wise growth may
    concentrate importance differently than XGBoost's level-wise strategy.
    The
  ...
  \item \textbf{Interventional SHAP under correlation.} DASH uses
    ...
    More broadly, \dash{} stabilizes \emph{predictive}
    attributions---what the model learned from each feature---not
    \emph{causal} attributions in the interventionist sense
    \citep{janzing2020feature}.  The term ``causally active'' used in
    Appendix~\ref{app:asymmetric} refers to features with non-zero
    coefficients in the data-generating process, not to a formal causal
    framework.  The semantics of interventional SHAP under collinearity
    are themselves disputed \citep{chen2020true, aas2021explaining};
    \dash{} does not resolve this debate but reduces the
    \emph{additional} instability introduced by model-training
    randomness.
  ...
  \item \textbf{Column ordering.} XGBoost's split selection can be
    influenced by the order in which features appear in the data matrix
    when candidate splits have equal gain.  We have not tested whether
    randomly permuting column order across repetitions affects the
    stability results.  Because \dash{} trains models with random
    \texttt{colsample\_bytree} subsets, column-order effects are
    partially mitigated by the stochastic feature sampling, but a
    systematic robustness test remains future work.
  \item \textbf{Sign stability.} This paper evaluates \emph{ranking}
    stability (Spearman correlation of importance magnitudes) but does
    not assess \emph{sign} stability---whether a feature's SHAP
    contribution is consistently positive or negative across retraining
    runs.  The companion impossibility theorem
    \citep{caraker2026impossibility} proves a bilemma showing that sign
    stability and faithfulness are jointly impossible for features in
    Rashomon pairs: no method can consistently assign the correct sign
    across equivalent models.  \dash{} consensus averaging resolves
    ranking instability (via the neutral element of tied ranks) but
    cannot resolve sign instability (binary signs admit no neutral
    element).  Empirical characterization of sign flip rates under
    \dash{} is deferred to future work.
\end{itemize}
...
A stronger result also holds: the companion paper proves a
\emph{bilemma} showing that even without the completeness requirement,
no method for determining a feature's SHAP sign (positive vs.\
negative contribution) can be simultaneously faithful and stable
across Rashomon-equivalent models.  The bilemma is constructive and
zero-axiom in Lean~4, requiring only the existence of two models with
identical predictions but opposite SHAP signs---a condition satisfied
by construction in any correlated feature group.  A corollary (the
all-or-nothing theorem) predicts that SHAP sign flip rates should
exhibit \emph{bimodality}: features either flip near 0\% (no
Rashomon ambiguity) or $\geq$50\% (full Rashomon ambiguity), with a
dead zone in between.  This prediction is directly testable against
the existing experimental data and constitutes a falsifiable
consequence of the theoretical framework.
\dash{} resolves \emph{ranking} instability because feature rankings
admit a neutral element (tied rank under consensus averaging), but the
bilemma proves that \emph{sign} instability is irreducible: binary
signs have no neutral element, so no averaging scheme can simultaneously
preserve faithfulness and achieve stability for sign attribution.
...
Apple M-series silicon, single node, and are reported for relative
comparison only.  Key software versions for the canonical results:
Python~3.11.11, XGBoost~2.1.4, SHAP~0.51.0, NumPy~2.3.5,
SciPy~1.14.1, scikit-learn~1.6.1, joblib~1.4.2.  Full environment
specifications (including BLAS/LAPACK configuration) are recorded in
\texttt{results/environment.json}.  We note that floating-point
non-determinism across hardware platforms (\eg SageMaker Intel vs.\
Apple~M-series ARM) can produce small numerical differences in SHAP
values at the least-significant digits; our stability conclusions are
robust to these differences because they are based on rank
correlations, which are invariant to monotone transformations of the
SHAP magnitudes.
...
\newblock \emph{arXiv preprint}, 2026. Lean~4 formalization (352 theorems, 6 axioms, 0 \texttt{sorry})
...
\paragraph{Colsample restriction ablation.}
\label{par:colsample_ablation}
To isolate whether forced low \texttt{colsample\_bytree} is the operative
diversity mechanism, we train \dash{} and Random Selection under three
\texttt{colsample\_bytree} ranges: Low (0.1--0.5, the default), High
(0.5--1.0), and Full (0.1--1.0), with Stochastic Retrain and Single Best
as controls.

At $\rhovar = 0$ (safety control), the colsample range has no effect:
\dash{} Low and High achieve identical stability ($0.972$ vs.\ $0.973$;
$p = 0.54$).  At $\rhovar = 0.9$, low colsample significantly outperforms
high colsample for both \dash{} ($0.976$ vs.\ $0.953$;
$\Delta = +0.023$, $p < 0.001$, bootstrap CIs non-overlapping) and
Random Selection ($0.977$ vs.\ $0.957$; $\Delta = +0.020$, $p < 0.001$).
Notably, \dash{} with high \texttt{colsample\_bytree} ($0.953$) performs
comparably to Single Best ($0.958$), indicating that ensemble averaging
\emph{without} forced feature restriction does not recover \dash{}-level
stability.  Under nonlinearity at $\rhovar = 0.9$, the gap widens:
\dash{} Low achieves $0.885$ vs.\ High $0.801$ ($\Delta = +0.084$;
bootstrap CIs non-overlapping).
...
\begin{enumerate}
  ...
  \item \textbf{Accuracy at $\rhovar = 0.9$}: DASH $\geq$ SB $\to$
    \textbf{PASS} (DASH $= 0.9881$ vs.\ SB $= 0.9782$)
...
All eleven criteria pass, which may suggest that the criteria were not
sufficiently stringent to risk failure on informative dimensions.  The
most revealing potential criterion would have been ``DASH stability $>$
SR stability (statistically significant),'' which our data does
\emph{not} support---SR matches or marginally exceeds DASH on stability
at every $\rhovar$ level.  We did not pre-specify this as a success
criterion because our hypothesis is that \emph{independence} drives
stability (which SR also achieves), not that \dash{}'s specific
pipeline outperforms SR.  Had we included ``DASH $>$ SR on stability,''
it would have failed---and this failure is itself informative, as it
constrains the contribution of MaxMin diversity selection to equity
and diagnostics rather than stability.  We encourage readers to treat
the SR equivalence as the most informative null result in this paper.
...
\paragraph{Limitations of this analytical model.}
This two-feature linear model demonstrates the \emph{seed mechanism}
(residual gain bias of $1/\rho^2$) but does not constitute a proof
that concentration occurs in full XGBoost under the paper's symmetric
DGP.  The bridge from the analytical seed to the observed concentration
is empirical: Figure~\ref{fig:t_scaling} shows monotonically growing
concentration as tree count increases in actual XGBoost, which is
consistent with the seed mechanism being amplified by the three factors
above.  A complete analytical model of concentration dynamics in
multi-depth, stochastic-feature-subset boosting---including the
interaction between \texttt{colsample\_bytree}, multi-level splits, and
residual evolution---remains an open theoretical problem.
...
\paragraph{Limitations of this validation.}
This demonstration uses only $M = 30$ models (vs.\ $M = 200$ in the
main experiments), yields $K_{\text{eff}} = 3$ due to low population
diversity under identical validation accuracy, and evaluates a single
dataset.  It confirms that the pipeline \emph{executes} on
non-TreeSHAP attributions and produces plausible results, but does not
constitute a rigorous evaluation of \dash{}'s stability properties
under LIME.  A full evaluation---with $M = 200$, multiple datasets,
and comparison against LIME-specific baselines---is needed before
claiming that \dash{} generalizes to non-TreeSHAP attribution methods.
