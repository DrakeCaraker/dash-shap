...
def test_linear_sweep_dash_stability_regression(v6):
    ...
    for rho in ["0.9", "0.95"]:
        ...
        assert abs(v7_val - v6_val) < TOLERANCE, (
            f"DASH stability at rho={rho}: v7={v7_val:.4f} vs v6={v6_val:.4f} (tolerance={TOLERANCE})"
        )
...
def test_breast_cancer_regression(v6):
    ...
    assert abs(v7_val - v6_val) < TOLERANCE * 2, f"Breast Cancer DASH: v7={v7_val:.4f} vs v6={v6_val:.4f}"
